...
A Lean `Type` is the universe of terms that can inhabit it. Here `Space d`
is the type of `d`-dimensional Euclidean points.
...
A basis packages canonical coordinates for vectors in a space. For `Space d`,
the standard basis indexed by `Fin d` is used below.
...
lemma basis_apply {d} (i j : Fin d) :
  ...
  rw [apply_eq_basis_repr_apply, OrthonormalBasis.repr_self, EuclideanSpace.single_apply]
...
noncomputable def coordCLM {d} (μ : Fin d) : Space d →L[ℝ] ℝ where
  ...
  cont := by
    simpa [coord] using (continuous_id.inner continuous_const)
...
lemma coord_contDiff {i} : ContDiff ℝ ∞ (fun x : Space d => x.coord i) := by
  simpa [coord] using
    (ContDiff.inner (𝕜 := ℝ)
      (hf := (contDiff_id : ContDiff ℝ ∞ (fun x : Space d => x)))
      (hg := (contDiff_const : ContDiff ℝ ∞ (fun _ : Space d => basis i))))
...
lemma oneEquiv_continuous :
  ...
  simpa using oneEquiv.continuous

lemma oneEquiv_symm_continuous :
    Continuous (oneEquiv.symm : ℝ → Space 1) := by
  simpa using oneEquiv.symm.continuous
...
noncomputable def oneEquivCLE : Space 1 ≃L[ℝ] ℝ where
  ...
  continuous_toFun := oneEquiv_continuous
  continuous_invFun := oneEquiv_symm_continuous

open MeasureTheory
lemma oneEquiv_measurableEmbedding : MeasurableEmbedding oneEquiv where
  injective := oneEquiv.injective
  measurable := oneEquiv_continuous.measurable
  measurableSet_image' := by
    intro s hs
    change MeasurableSet (⇑oneEquivCLE '' s)
    rw [Set.image_eq_preimage_of_inverse oneEquivCLE.symm_apply_apply oneEquivCLE.apply_symm_apply]
    exact oneEquivCLE.symm.continuous.measurable hs

lemma oneEquiv_symm_measurableEmbedding : MeasurableEmbedding oneEquiv.symm where
  injective := oneEquiv.symm.injective
  measurable := oneEquiv_symm_continuous.measurable
  measurableSet_image' := by
    intro s hs
    change MeasurableSet (⇑oneEquivCLE.symm '' s)
    rw [Set.image_eq_preimage_of_inverse oneEquivCLE.symm.symm_apply_apply
      oneEquivCLE.symm.apply_symm_apply]
    exact oneEquivCLE.continuous.measurable hs
